...
module.exports = function (runtime, cb) {
  ...
  if (utils.satisfied(runtime, 'ruby', runtime.binDir + '/ruby -v', 'ruby-shim')) {
    ...
    if (process.env.DOCKER_BUILD === '1') {
      var cache = process.env.DOCKER_RESET === '1' ? ' --no-cache' : '';
  ...
  if (rubyProvider !== 'docker') {
    ...
    if (os.platform() === 'darwin' && shell.exec('brew -v', { 'silent': true }).code === 0) {
      Scrolex.exe(['cd', runtime.cacheDir, '&&', '(', 'brew install libxml2;', runtime.prerequisites.bundler.exe, 'config build.nokogiri', '--use-system-libraries', '--with-xml2-include=$(brew --prefix libxml2 | sed \'s@_[0-9]*$@@\')/include/libxml2' + runtime.prerequisites.ruby.exeSuffix, ')'].join(' '), { components: 'lanyon>postinstall>bundler>configure' });
    } else {
      Scrolex.exe(['cd', runtime.cacheDir, '&&', runtime.prerequisites.bundler.exe, 'config build.nokogiri', '--use-system-libraries' + runtime.prerequisites.ruby.exeSuffix].join(' '), { components: 'lanyon>postinstall>bundler>configure' });
    }

    runtime.prerequisites.jekyll.exe = runtime.prerequisites.bundler.exe + ' exec jekyll';

    // Install Gems from Gemfile bundle
    Scrolex.exe(['cd', runtime.cacheDir, '&&', '(', runtime.prerequisites.bundler.exe, 'install', '--binstubs=\'' + runtime.binDir + '\'', '--path \'vendor/bundler\'' + runtime.prerequisites.ruby.exeSuffix, '||', runtime.prerequisites.bundler.exe, 'update' + runtime.prerequisites.ruby.exeSuffix, ')'].join(' '), { components: 'lanyon>postinstall>gems>install' });
  }
...
var _temp = function () {
  ...
  __REACT_HOT_LOADER__.register(yes, 'yes', 'src/postinstall.js');
}();
