...
var SvgStoreWebpackPlugin = require('webpack-svgstore-plugin');
...
var cfg = {
  webpack: {
    ...
    plugins: function plugins() {
      var plugins = [new BowerWebpackPlugin(), new webpack.ProvidePlugin({
        _: 'lodash',
        $: 'jquery',
        jQuery: 'jquery'
    ...
    resolveLoader: {
      root: [path.join(runtime.lanyonDir, 'node_modules'), path.join(runtime.projectDir, 'node_modules')]
    },
    recordsPath: runtime.recordsPath,
    resolve: {
      root: [path.resolve(runtime.assetsSourceDir), path.resolve(runtime.assetsSourceDir) + '/bower_components', path.resolve(runtime.projectDir) + '/node_modules', path.resolve(runtime.lanyonDir) + '/node_modules']
    },
    debug: runtime.isDev
  }
};
...
cfg.jekyll = {
  exclude: ['node_modules', 'env.sh', 'env.*.sh', '.env.sh', '.env.*.sh', '.lanyon']
};
...
var _temp = function () {
  ...
  __REACT_HOT_LOADER__.register(scrolex, 'scrolex', 'src/config.js');

  __REACT_HOT_LOADER__.register(runtime, 'runtime', 'src/config.js');

  __REACT_HOT_LOADER__.register(jekyllConfigPath, 'jekyllConfigPath', 'src/config.js');

  __REACT_HOT_LOADER__.register(allApps, 'allApps', 'src/config.js');

  __REACT_HOT_LOADER__.register(getFilename, 'getFilename', 'src/config.js');

  __REACT_HOT_LOADER__.register(cfg, 'cfg', 'src/config.js');

  __REACT_HOT_LOADER__.register(bundler, 'bundler', 'src/config.js');
}();
