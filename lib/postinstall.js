...
module.exports = function (runtime, cb) {
  ...
  if (utils.satisfied(runtime, 'ruby', runtime.binDir + '/ruby -v', 'ruby-shim')) {
    ...
    if (process.env.DOCKER_BUILD === '1') {
      ...
      utils.fatalExe('cd .lanyon && docker build' + cache + ' -t kevinvz/lanyon:' + runtime.lanyonVersion + ' .');
  ...
  if (rubyProvider !== 'docker') {
    ...
    if (os.platform() === 'darwin' && shell.exec('brew -v', { 'silent': true }).code === 0) {
      utils.fatalExe(['cd', runtime.cacheDir, '&&', '(', 'brew install libxml2;', runtime.prerequisites.bundler.exe, 'config build.nokogiri', '--use-system-libraries', '--with-xml2-include=$(brew --prefix libxml2 | sed \'s@_[0-9]*$@@\')/include/libxml2' + runtime.prerequisites.ruby.exeSuffix, ')']);
    } else {
      utils.fatalExe(['cd', runtime.cacheDir, '&&', runtime.prerequisites.bundler.exe, 'config build.nokogiri', '--use-system-libraries' + runtime.prerequisites.ruby.exeSuffix]);
    }
    ...
    utils.fatalExe(['cd', runtime.cacheDir, '&&', '(', runtime.prerequisites.bundler.exe, 'install', '--binstubs=\'' + runtime.binDir + '\'', '--path \'vendor/bundler\'' + runtime.prerequisites.ruby.exeSuffix, '||', runtime.prerequisites.bundler.exe, 'update' + runtime.prerequisites.ruby.exeSuffix, ')']);
  }
