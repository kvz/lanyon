...
module.exports = function (runtime, cb) {
  ...
  if (rubyProvider !== 'docker') {
    ...
    if (os.platform() === 'darwin' && shell.exec('brew -v', { 'silent': true }).code === 0) {
      utils.fatalExe(['cd', runtime.cacheDir, '&&', '(', 'brew install libxml2;', runtime.prerequisites.bundler.exe, 'config build.nokogiri', '--use-system-libraries', '--with-xml2-include=$(brew --prefix libxml2)/include/libxml2' + runtime.prerequisites.ruby.exeSuffix, ')']);
    } else {
