...
module.exports = function (runtime, cb) {
  ...
  if (utils.satisfied(runtime, 'ruby', '' + runtime.binDir + '/ruby -v', 'ruby-shim')) {
    ...
    if (process.env.DOCKER_BUILD === '1') {
      utils.fatalExe('docker build -t kevinvz/lanyon:' + runtime.lanyonVersion + ' .')
  ...
  fs.writeFileSync(shimPath, '#!/bin/sh\n(npm run build:production || npm run web:build:production) && npm run deploy', { 'encoding': 'utf-8', 'mode': '755' })
  console.log(yes)
